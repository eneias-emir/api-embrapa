...
from utils import url_to_csv_filename
...
class LoadData:
    ...
    def gravar_reg(self, linha: list, reg: dict, ind_inicio_ano: int, codigo: str, descricao: str, importacao_exportacao: bool = False) -> None:
    ...
    def gravar_linha(self, item_config: dict, linha: list) -> None:
        ...
        if opt == OPT_PRODUCAO:
            ...
            self.gravar_reg(linha, reg, ind_inicio_ano=3, codigo=linha[1], descricao=linha[2])
        elif opt == OPT_IMPORTACAO or opt == OPT_EXPORTACAO:
            self.gravar_reg(linha, reg, ind_inicio_ano=2, codigo='', descricao=linha[1], importacao_exportacao=True)
